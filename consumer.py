...
def store_cassandra_raw(batch_df, batch_id):
    ...
    try:
        ...
        cluster = Cluster(['cassandra.finstream.svc.cluster.local'], port=9042)
...
def consumer():
    ...
    try:
        ...
        spark = SparkSession.builder.appName("FinStreamProcessing").config("spark.jars.packages", "org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.1,com.datastax.spark:spark-cassandra-connector_2.12:3.5.1,com.github.jnr:jnr-posix:3.1.15").config("spark.cassandra.connection.host", "cassandra.finstream.svc.cluster.local").config("spark.cassandra.connection.port", "9042").getOrCreate()
        ...
        df = spark.readStream.format('kafka').option("kafka.bootstrap.servers", 'kafka:9092').option("subscribe", 'finnhub_trades').option("startingOffsets", "earliest").option("group.id", "finstream-group").load()
